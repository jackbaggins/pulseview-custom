...
Session::Session(DeviceManager &device_manager, QString name)
    : shutting_down_(false), device_manager_(device_manager),
      default_name_(name), name_(name), capture_state_(Stopped),
      cur_samplerate_(0), data_saved_(true) {
...
DeviceManager &Session::device_manager() { return device_manager_; }

const DeviceManager &Session::device_manager() const { return device_manager_; }
...
shared_ptr<devices::Device> Session::device() const { return device_; }

QString Session::name() const { return name_; }
...
QString Session::save_path() const { return save_path_; }

void Session::set_save_path(QString path) { save_path_ = path; }

const vector<shared_ptr<views::ViewBase>> Session::views() const {
  return views_;
}

shared_ptr<views::ViewBase> Session::main_view() const { return main_view_; }
...
bool Session::data_saved() const { return data_saved_; }
...
void Session::save_setup(QSettings &settings) const {
  ...
  for (const shared_ptr<data::SignalBase> &base : signalbases_) {
    ...
    if (base->is_decode_signal()) {
      settings.beginGroup("decode_signal" +
                          QString::number(decode_signal_count++));
      base->save_settings(settings);
      settings.endGroup();
    } else
#endif
        if (base->is_generated()) {
      settings.beginGroup("generated_signal" +
                          QString::number(gen_signal_count++));
  ...
  for (const shared_ptr<views::ViewBase> &vb : views_) {
    shared_ptr<views::trace::View> tv =
        dynamic_pointer_cast<views::trace::View>(vb);
    if (tv) {
      for (const shared_ptr<views::trace::TimeItem> &time_item :
           tv->time_items()) {
      ...
      if (tv->cursors_shown()) {
        ...
        GlobalSettings::store_timestamp(settings, "start_time",
                                        cp->first()->time());
        GlobalSettings::store_timestamp(settings, "end_time",
                                        cp->second()->time());
        settings.endGroup();
      }
    }
...
void Session::restore_setup(QSettings &settings) {
  ...
  for (int i = 0; i < gen_signal_count; i++) {
    ...
    SignalBase::ChannelType type =
        (SignalBase::ChannelType)settings.value("type").toInt();
  ...
  for (int i = 0; i < meta_objs; i++) {
    ...
    if ((type == "selection") && tv) {
      Timestamp start =
          GlobalSettings::restore_timestamp(settings, "start_time");
...
void Session::restore_settings(QSettings &settings) {
  ...
  if ((device_type == "sessionfile") || (device_type == "inputfile")) {
    ...
    if (device_type == "inputfile") {
      settings.beginGroup("device");
      device =
          make_shared<devices::InputFile>(device_manager_.context(), settings);
      settings.endGroup();
    }
    ...
    if (device) {
      ...
      set_name(QString::fromStdString(
          dynamic_pointer_cast<devices::File>(device)->display_name(
              device_manager_)));
...
void Session::set_device(shared_ptr<devices::Device> device) {
  ...
  try {
    ...
    device_.reset();
    MainWindow::show_session_error(tr("Failed to open device"),
                                   QString(e.what()));
  }
  ...
  if (!format.empty()) {
    ...
    if (iter == formats.end()) {
      MainWindow::show_session_error(tr("Error"),
                                     tr("Unexpected input format: %1")
                                         .arg(QString::fromStdString(format)));
      return;
    }
    input_format = (*iter).second;
    input_opts = input_format_options(user_opts, input_format->options());
  }

  load_file(QString::fromStdString(file_name),
            QString::fromStdString(setup_file_name), input_format, input_opts);
}

void Session::load_file(QString file_name, QString setup_file_name,
                        shared_ptr<sigrok::InputFormat> format,
                        const map<string, Glib::VariantBase> &options) {
  const QString errorMessage(QString("Failed to load file %1").arg(file_name));

  // In the absence of a caller's format spec, try to auto detect.
  // Assume "sigrok session file" upon lookup miss.
  if (!format)
    format =
        device_manager_.context()->input_format_match(file_name.toStdString());
  try {
    if (format)
      set_device(shared_ptr<devices::Device>(
          new devices::InputFile(device_manager_.context(),
                                 file_name.toStdString(), format, options)));
    else
      set_device(shared_ptr<devices::Device>(new devices::SessionFile(
          device_manager_.context(), file_name.toStdString())));
  } catch (Error &e) {
    MainWindow::show_session_error(tr("Failed to load %1").arg(file_name),
                                   e.what());
    return;
  }
  ...
  if (QFileInfo::exists(setup_file_name) &&
      QFileInfo(setup_file_name).isReadable()) {
...
void Session::start_capture(function<void(const QString)> error_handler) {
  ...
  Glib::TimeVal acquisition_start_time_;

  Glib::DateTime dt = Glib::DateTime::create_now_local();
  acquisition_start_time_.tv_sec = dt.to_unix();
  acquisition_start_time_.tv_usec = dt.get_microsecond();

  // Begin the session
  sampling_thread_ =
      std::thread(&Session::sample_thread_proc, this, error_handler);
}
...
void Session::register_view(shared_ptr<views::ViewBase> view) {
  ...
  for (const shared_ptr<data::SignalBase> &signalbase : signalbases_) {
    const int sb_exists =
        count_if(view_signalbases.cbegin(), view_signalbases.cend(),
...
double Session::get_samplerate() const {
  ...
  for (const shared_ptr<pv::data::SignalData> &d : all_signal_data_) {
    assert(d);
    const vector<shared_ptr<pv::data::Segment>> segments = d->segments();
...
Glib::TimeVal Session::get_acquisition_start_time() const {
  return acq_start_time_;
}

uint32_t Session::get_highest_segment_id() const { return highest_segment_id_; }
...
uint32_t Session::get_signal_count(data::SignalBase::ChannelType type) const {
  return count_if(
      signalbases_.begin(), signalbases_.end(),
...
void Session::update_signals() {
  ...
  unsigned int logic_channel_count = count_if(
      channels.begin(), channels.end(), [](shared_ptr<Channel> channel) {
    ...
    if (iter == signalbases_.cend()) {
      ...
      case SR_CHANNEL_LOGIC:
        signalbase = make_shared<data::SignalBase>(
            channel, data::SignalBase::LogicChannel);
        signalbases_.push_back(signalbase);

        all_signal_data_.insert(logic_data_);
        signalbase->set_data(logic_data_);

        connect(this, SIGNAL(capture_state_changed(int)), signalbase.get(),
                SLOT(on_capture_state_changed(int)));
        break;

      case SR_CHANNEL_ANALOG:
        signalbase = make_shared<data::SignalBase>(
            channel, data::SignalBase::AnalogChannel);
        signalbases_.push_back(signalbase);
  ...
  for (shared_ptr<views::ViewBase> &viewbase : views_) {
    vector<shared_ptr<SignalBase>> view_signalbases = viewbase->signalbases();
...
Session::signalbase_from_channel(shared_ptr<sigrok::Channel> channel) const {
...
void Session::sample_thread_proc(function<void(const QString)> error_handler) {
  ...
  sink_ = RefPtr<AppSink>::cast_dynamic(
      ElementFactory::create_element("appsink", "sink"));
  ...
  sink_->signal_new_sample().connect(
      sigc::mem_fun(*this, &Session::on_gst_new_sample));
...
#ifdef ENABLE_FLOW
bool Session::on_gst_bus_message(const Glib::RefPtr<Gst::Bus> &bus,
...
Gst::FlowReturn Session::on_gst_new_sample() {
  ...
  for (uint32_t block_id = 0; block_id < buf->n_memory(); block_id++) {
    ...
    try {
      feed_in_logic(
          dynamic_pointer_cast<sigrok::Logic>(logic_packet->payload()));
    } catch (bad_alloc &) {
      out_of_memory_ = true;
      device_->stop();
...
void Session::feed_in_meta(shared_ptr<Meta> meta) {
  for (auto &entry : meta->config()) {
    ...
    case SR_CONF_SAMPLERATE:
      ...
      break;
    default:
      qDebug() << "Received meta data key" << entry.first->id()
               << ", ignoring.";
      break;
    }
  }
...
void Session::feed_in_trigger() {
  ...
  // TODO Create timestamp from segment start time + segment's current sample
  // count
...
void Session::feed_in_frame_begin() { frame_began_ = true; }
...
void Session::feed_in_logic(shared_ptr<sigrok::Logic> logic) {
  ...
  if (!cur_logic_segment_) {
    ...
    cur_logic_segment_ = make_shared<data::LogicSegment>(
        *logic_data_, logic_data_->get_segment_count(), logic->unit_size(),
        cur_samplerate_);
  ...
  segment_sample_count_[highest_segment_id_] =
      max(segment_sample_count_[highest_segment_id_],
          cur_logic_segment_->get_sample_count());
...
void Session::feed_in_analog(shared_ptr<sigrok::Analog> analog) {
  ...
  for (auto &channel : channels) {
    ...
    const map<shared_ptr<Channel>, shared_ptr<data::AnalogSegment>>::iterator
        iter = cur_analog_segments_.find(channel);
...
void Session::on_data_saved() { data_saved_ = true; }
